...
1. **Real payment integration (Stripe Elements).** The web checkout still uses the mock token flow. The API is production-ready for Stripe (`PAYMENT_PROVIDER_KEY=sk_*` switches providers and env validation enforces it in production), but the frontend has no card-entry element to produce a real `paymentToken`. Until then set `NEXT_PUBLIC_PAYMENT_MODE` to something other than `mock` in production to disable online checkout.
...
- **Infra**: private R2 bucket with presigned URLs, Prisma-only DB access (no raw SQL with user input), Zod env validation that refuses to boot production without a real Stripe key, `trust proxy`, helmet, restricted CORS, nginx rate limiting + security headers.
