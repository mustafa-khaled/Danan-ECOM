...
import { ApiError, fetchCollection } from "../../../../lib/api";
    ...
    <PrivateLayout clientName={profile.displayName} navItems={privateNavItems}>
      <nav aria-label="Breadcrumb" className="mb-6 text-xs tracking-[0.12em] uppercase">
        <ol className="flex flex-wrap items-center gap-2 text-[var(--color-ivory-muted)]">
          <li>
            <Link href="/home" className="hover:text-[var(--color-gold-light)]">
              Home
            </Link>
          </li>
          <li aria-hidden="true">/</li>
          <li>
            <Link href="/collections" className="hover:text-[var(--color-gold-light)]">
              Collections
            </Link>
          </li>
          <li aria-hidden="true">/</li>
      ...
      {collection.designs.length === 0 ? (
        <EmptyState
          ...
          action={{ href: "/collections", label: "Back to Collections" }}
        />
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {collection.designs.map((design) => (
            <Link key={design.id} href={`/pieces/${design.slug}`}>
              <PieceCard
                piece={{
                  id: design.id,
                  name: design.name,
