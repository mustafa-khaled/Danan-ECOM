...
export class ParseFloatPipe implements PipeTransform<string, number> {
  constructor(
    private options?: { optional?: boolean; min?: number; max?: number },
  ) {}
...
export class UsersController {
  ...
  @Get()
  findAll(
    @Query("page", new ParseIntPipe({ optional: true, minValue: 1 }))
    page?: number,
    @Query(
      "limit",
      new ParseIntPipe({ optional: true, minValue: 1, maxValue: 100 }),
    )
    limit?: number,
    @Query("active", new ParseBooleanPipe({ optional: true })) active?: boolean,
    @Query("search", TrimPipe) search?: string,
...
describe("ParseIntPipe", () => {
  ...
  it("should throw for invalid string", () => {
    expect(() =>
      pipe.transform("abc", { type: "query", data: "page" } as any),
  ...
  it("should enforce min value", () => {
    ...
    expect(() =>
      pipeWithMin.transform("0", { type: "query", data: "page" } as any),
    ).toThrow(BadRequestException);
  });
  ...
  it("should enforce max value", () => {
    ...
    expect(() =>
      pipeWithMax.transform("101", { type: "query", data: "page" } as any),
    ).toThrow(BadRequestException);
  });
});
```
