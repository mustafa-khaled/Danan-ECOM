...
export class OrderSaga {
  ...
  async handleOrderCreated(event: OrderCreatedEvent) {
    ...
    if (paymentResult.success) {
      ...
      this.eventEmitter.emit(
        "payment.completed",
        new PaymentCompletedEvent(event.orderId, paymentResult.amount),
      );
    } else {
  ...
  async handlePaymentCompleted(event: PaymentCompletedEvent) {
    ...
    if (inventoryResult.success) {
      this.eventEmitter.emit(
        "inventory.reserved",
        new InventoryReservedEvent(event.orderId, inventoryResult.items),
      );
    } else {
      // Refund payment
...
export class UserRegisteredEventV2 {
  ...
  static fromV1(v1Event: UserRegisteredEvent): UserRegisteredEventV2 {
    return new UserRegisteredEventV2(v1Event.userId, v1Event.email, {
      source: "migration",
      version: "1->2",
    });
  }
