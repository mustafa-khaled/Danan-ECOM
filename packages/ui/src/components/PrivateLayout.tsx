      ...
      <header className="border-b border-[var(--color-border)]">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-6 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <div>
            ...
            <p className="mt-1 text-xs tracking-[0.16em] uppercase text-[var(--color-ivory-muted)]">
              Private House
            </p>
          </div>
          ...
          <ul className="flex flex-wrap gap-2">
            {navItems.map((item) => (
              <li key={item.href}>
                <a
                  href={item.href}
                  className="inline-flex min-h-11 items-center rounded-[var(--radius-item)] border border-transparent px-4 text-xs tracking-[0.12em] uppercase text-[var(--color-ivory-muted)] transition-colors hover:border-[var(--color-border)] hover:text-[var(--color-gold-light)] focus-visible:shadow-[var(--shadow-focus)]"
                >
                  {item.label}
                </a>
              </li>
