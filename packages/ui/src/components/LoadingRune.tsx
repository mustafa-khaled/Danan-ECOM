...
export function LoadingRune({ label = "Loading", className = "" }: LoadingRuneProps) {
  return (
    <div
      role="status"
      aria-live="polite"
      className={["flex flex-col items-center gap-4", className].filter(Boolean).join(" ")}
    >
      ...
      <span className="font-display text-sm tracking-[0.2em] uppercase text-[var(--color-gold-light)]">
        {label}
      </span>
    </div>
  );
